...
type TemplateBaseExtensions<'T>() =
    ...
    member x.RenderPart(templateName, model:obj) =  
        ...
        let file =
          ...
          | Some root when File.Exists(Path.Combine(root, "parts", templateName + ".cshtml")) ->  
              Path.Combine(root, "parts", templateName + ".cshtml")
          | _ -> failwith "Template file %s not found." templateName
